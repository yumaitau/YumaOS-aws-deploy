...
5. The published YumaOS web image always checks out a Marketplace license with License Manager `CheckoutLicense` every 15 minutes. Hermes is a public sidecar and is not license-gated. Task-definition environment variables cannot disable the web check. Product identity is baked into the web image at build. `MarketplaceProductCode` / `MarketplaceProductSku` on the stack are documentary.
